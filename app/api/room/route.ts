...
export async function POST(req:Request){
  ...
  if(action==="move"){
    ...
    s.craters=s.craters??[];const angle=Math.max(18,Math.min(78,Number(p.angle)||45)),power=Math.max(1,Math.min(100,rawPower)),shotWind=s.wind,target=(1-player) as 0|1,terrain=(s.turnNo-1)%4;
