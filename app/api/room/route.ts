...
export async function POST(req:Request){
  ...
  if(action==="move"){
    ...
    s.craters=s.craters??[];const angle=Math.max(18,Math.min(78,Number(p.angle)||45)),power=Math.max(1,Math.min(100,rawPower)),shotWind=s.wind,target=(1-player) as 0|1;
    ...
    const expired=s.turn,target=(1-expired) as 0|1;s.turnsTaken[expired]++;s.turn=target;s.turnNo++;s.turnStartedAt=Date.now();s.moved[target]=0;let windChanged=false;if((s.turnNo-1)%4===0){s.wind=Math.round(Math.random()*28-14);windChanged=true}s.lastEvent={type:"timeout",player:expired,windChanged,nonce:Date.now()};
