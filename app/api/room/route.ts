...
type GameState={positions:[number,number];hp:[number,number];turn:0|1;turnNo:number;moved:[number,number];specialUsed:[boolean,boolean];wind:number;winner:number|null;lastEvent:{type:string;player?:number;from?:number;to?:number;angle?:number;power?:number;special?:boolean;hit?:boolean;damage?:number;nonce:number};chat:ChatLine[]};
const clean=(v:unknown,n=80)=>String(v??"").trim().slice(0,n);
const fresh=():GameState=>({positions:[122,638],hp:[100,100],turn:0,turnNo:1,moved:[0,0],specialUsed:[false,false],wind:Math.round(Math.random()*16-8),winner:null,lastEvent:{type:"start",nonce:Date.now()},chat:[{who:"Rana",text:"Ya pe… no llores cuando pierdas. 🐸"}]});
...
export async function POST(req:Request){
  ...
  if(action==="join"){
    ...
    if(row.guest_token&&row.guest_token!==token)return Response.json({error:"Room full. Dos nomás."},{status:409});
  ...
  if(player<0)return Response.json({error:"No eres de este barrio."},{status:403});
  const s=JSON.parse(row.state) as GameState;
  if(action==="move"){
    if(s.turn!==player||s.winner!==null)return Response.json({error:"Ahorita no, campeón."},{status:409});
    const requested=Math.max(-34,Math.min(34,Number(p.delta)||0));const left=72-s.moved[player];const delta=Math.sign(requested)*Math.min(Math.abs(requested),left);
    ...
    if(s.turn!==player||s.winner!==null)return Response.json({error:"Ahorita no, campeón."},{status:409});
    const special=Boolean(p.special);if(special&&s.specialUsed[player])return Response.json({error:"El especial ya fue, pe."},{status:409});
    const angle=Math.max(18,Math.min(78,Number(p.angle)||45)),power=Math.max(28,Math.min(100,Number(p.power)||60));let x=s.positions[player],y=263,dx=Math.cos(angle*Math.PI/180)*power*.145*(player===0?1:-1),dy=-Math.sin(angle*Math.PI/180)*power*.145;
    for(let i=0;i<420&&y<310&&x>0&&x<760;i++){x+=dx;y+=dy;dy+=.17;dx+=s.wind*.0009*(player===0?1:-1)}
    ...
    s.lastEvent={type:"fire",player,angle,power,special,hit,damage,nonce:Date.now()};
