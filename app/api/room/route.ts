...
type GameState={positions:[number,number];hp:[number,number];turn:0|1;turnNo:number;roundNo:number;scene:number;nextScene:number|null;matchWins:[number,number];characters:[CharacterId,CharacterId];turnStartedAt:number;moved:[number,number];turnsTaken:[number,number];specialReadyAt:[number,number];itemUsed:[boolean,boolean];wind:number;winner:number|null;craters:Crater[];lastEvent:{type:string;player?:number;from?:number;to?:number;impactX?:number;secondImpactX?:number;angle?:number;power?:number;wind?:number;windChanged?:boolean;special?:boolean;dual?:boolean;hit?:boolean;damage?:number;terrain?:number;nonce:number};chat:ChatLine[]};
...
const fresh=(matchWins:[number,number]=[0,0],roundNo=1,characters:[CharacterId,CharacterId]=[...DEFAULT_CHARACTERS],scene=randomSceneIndex()):GameState=>({positions:[122,638],hp:[160,160],turn:0,turnNo:1,roundNo,scene,nextScene:null,matchWins,characters:[...characters],turnStartedAt:Date.now(),moved:[0,0],turnsTaken:[0,0],specialReadyAt:[0,0],itemUsed:[false,false],wind:Math.round(Math.random()*28-14),winner:null,craters:[],lastEvent:{type:"start",nonce:Date.now()},chat:[{who:characterById(characters[1]).name,text:rivalQuips[Math.floor(Math.random()*rivalQuips.length)]}]});
...
export async function POST(req:Request){
  ...
  if(action==="select"){
    ...
    s.craters=s.craters??[];const angle=Math.max(18,Math.min(78,Number(p.angle)||45)),power=Math.max(1,Math.min(100,rawPower)),shotWind=s.wind,target=(1-player) as 0|1,terrain=s.scene;
